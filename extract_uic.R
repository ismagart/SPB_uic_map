...
uic_reader <- function(matrix_uic, stop_word_patern, street_name_pattern){
gg <- data.frame(uic = character(2500), 
                 start_position = integer(2500),
                 end_position = integer(2500),
                 adres = character(2500),
...
while(i <= length(matrix_uic[,1])){
  ...
  if(grepl("ТИК", matrix_uic[i,1]) & G == TRUE){
    ...
    gg[k,8] <- str_extract(full_adres_string,
                           "((лит\\.|литера|литер)(\\W*)(А|Б|В|Г))|(\\d(а|б|в|г|А|Б|В|Г))|(\\d[ ](а|б|в|г))")
...
system.time(gg <- uic_reader(clear_uic_matrix, stop_word_patern, street_name_pattern))

# Разбираемся с островами
...
# 6) проблемы с островами
# 7) создать поселки/деревни/территории
# 8) улицы без идентификатора что это улицы или проспекты
# 9) Проблема "Больших" улиц. Б.Пароховской, Б.Морской, Б. Сампсониевский
